package test;
import main.ActivityNetwork
import main.ActivityNode;

class ActivityNetworkTest extends GroovyTestCase {
    /**
    ...
    void testNodeInsertion() {
        ...
        assertEquals(0, n.getNodeList()[0].getNodeId());
     ...
     */
    void testNodesSortedAfterModification() {
        ActivityNetwork n = new ActivityNetwork(0);
        ...
        a.setDependencies(new HashSet<Long>([3]));
        b.setDependencies(new HashSet<Long>([9]));
        n.insertNode(a);
        n.insertNode(b);

        assertEquals(9, n.getStartNodeId());
        assertEquals(7, n.getNodeList()[2].getNodeId());

        // 7 now depends on 9, 9 is start node.
        n.deleteNode(3);

        assertEquals(9, n.getStartNodeId());
        assertEquals(7, n.getNodeList()[1].getNodeId());
    }
}
